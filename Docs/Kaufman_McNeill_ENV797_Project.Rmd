...
```{r create exogenous variables for ETS}
...
```{r ETS with exogenous variable for SAL}
...
##PICK UP HERE
#LT2 ETS with rainfall regressor
...
monthly_reg <- auser_raw %>%
  ...
  group_by(Month) %>%
  summarize_all(mean, na.rm = TRUE) %>%
  select(Month, LT2, Temperature_Lucca_Orto_Botanico, Rainfall_Gallicano) %>%
  slice(110:268)
