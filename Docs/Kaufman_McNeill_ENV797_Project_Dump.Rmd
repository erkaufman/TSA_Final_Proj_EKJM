...
**NOTE: making these cleaned time series here for exogenous variables wasn't actually necessary. we had to remake these later when we modeled with exogenous variables because we had to make it so that all the time series started at the same time.
...
#ADD A KBL HERE WITH THE TREND RESULTS LIKE THE TABLE WE MADE IN THE SLIDES
...
monthly_reg <- auser_raw %>%
  ...
  group_by(Month) %>%
  summarize_all(mean, na.rm = TRUE) %>%
  select(Month, LT2, SAL, Temperature_Lucca_Orto_Botanico, Rainfall_Monte_Serra) %>%
  slice(110:268)
...
rain_reg_monthly <- monthly_reg %>%
  select(Month, Rainfall_Monte_Serra)
...
```{r using both temp and rainfall for SAL}
regressors_train <- as.matrix(data.frame("rain"=rain_monthly_training_reg, "temp"=temp_monthly_training_reg))

regressors_test <- as.matrix(data.frame("rain"=rain_monthly_testing_reg, "temp"=temp_monthly_testing_reg))

regressors_all <-as.matrix(data.frame("rain"=ts_rain_monthly_reg, "temp"=ts_temp_monthly_reg))

#SAL auto sarima with both regressors
auto_SAL_temp_reg_2_train <- auto.arima(SAL_monthly_training_reg, xreg=regressors_train)

auto_SAL_temp_reg_2_test <- forecast(auto_SAL_temp_reg_2_train, h=12,  xreg=regressors_train)

#plot foresting results
autoplot(auto_SAL_temp_reg_2_test)+ 
  ylab("depth to groundwater (meters)") + 
  ggtitle("Auto Sarima SAL with Temperature Regressor") +
  theme_light()
...
#plot model + observed data
autoplot(ts_SAL_monthly_reg, series = "Original") +
  autolayer(auto_SAL_temp_reg_2_test, series = "Auto Sarima W/ Reg", PI = FALSE) +
  ylab("depth to groundwater (meters)") +
  ggtitle("Auto Sarima SAL with Temperature Regressor") +
  theme_light()

#check the MAPE
auto_SAL_temp_reg_scores <- accuracy(auto_SAL_temp_reg_2_test$mean, SAL_monthly_testing_reg)
print(auto_SAL_temp_reg_scores)
...
```{r applying best models to the whole dataset and forecasting into the future}
...
```{r create exogenous variables for ETS jenn}

# #subset columns of interest
# daily_reg <- auser_raw %>%
#   select(Date:Temperature_Lucca_Orto_Botanico) %>%
#   rename(SAL = Depth_to_Groundwater_SAL) %>%
#   select(Date, SAL, Rainfall_Gallicano:Rainfall_Fabbriche_di_Vallico, Temperature_Lucca_Orto_Botanico) %>%
#   slice(3320:8154)
# 
# #create a dataframe for each regressor that starts at the unique first month of data
# temp_reg_daily <- daily_reg %>%
#   select(Date, Temperature_Lucca_Orto_Botanico)
# 
# rain_reg_daily <- daily_reg %>%
#   select(Date, Rainfall_Gallicano) #change out this variable to test diff rain regressor
# 
# SAL_for_reg <- daily_reg %>%
#   select(Date, SAL)
# 
# #create time series objects for monthly averages
# ts_SAL_daily_reg <- tsclean(ts(SAL_for_reg[,2],start=c(2007,04,06),frequency=365))
# ts_rain_daily_reg <- tsclean(ts(rain_reg_daily[,2],start=c(2007,04,06),frequency=365))
# ts_temp_daily_reg <- tsclean(ts(temp_reg_daily[,2],start=c(2007,04,06),frequency=365))
# 
# #plot time series and regressors for visualization
# autoplot(ts_SAL_daily_reg) +
#   autolayer(ts_rain_daily_reg) +
#   autolayer(ts_temp_daily_reg) +
#   theme_light()
```

```{r ETS with exogenous variables for SAL jenn}

#create training and testing sets with the exogenous variables
# n_for = 365
# 
# SAL_daily_training_reg <- subset(ts_SAL_daily_reg, end = length(ts_SAL_daily_reg) - n_for)
# 
# SAL_daily_testing_reg <- subset(ts_SAL_daily_reg, start = length(ts_SAL_daily_reg) - n_for)
# 
# rain_daily_training_reg <- subset(ts_rain_daily_reg, end = length(ts_rain_daily_reg) - n_for)
# 
# rain_daily_testing_reg <- subset(ts_rain_daily_reg, start = length(ts_rain_daily_reg) - n_for)
# 
# temp_daily_training_reg <- subset(ts_temp_daily_reg, end = length(ts_temp_daily_reg) - n_for)
# 
# temp_daily_testing_reg <- subset(ts_temp_daily_reg, start = length(ts_temp_daily_reg) - n_for)
# 
# #SAL ETS with rainfall regressor
# # I DONT THINK THAT YOU CAN RUN AN ETF WITH AN EXTERNAL REGRESSOR PERIOD?
# # stlm_rain_df <- data.frame(SAL_daily_training_reg, rain_daily_training_reg)
# #
# # ets_SAL_rain_reg_train <- stlm(SAL_daily_training_reg ~ rain_daily_training_reg, data=stlm_rain_df)
# #
# # ets_SAL_rain_reg_train <- stlm(SAL_daily_training_reg ~ rain_daily_training_reg)
# #
# # stlf(SAL_daily_training_reg,xreg=rain_daily_training_reg)
# 
# auto_LT2_rain_reg_test <- forecast(auto_LT2_rain_reg_train, h=12,  xreg=rain_monthly_testing_reg)
# 
# #plot foresting results
...
```{r ETS with exogenous variables for SAL emma}
