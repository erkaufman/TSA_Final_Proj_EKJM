...
# n_for_month = 30
# LT2_daily_training2 <- subset(ts_LT2_clean, end = length(ts_LT2_clean) - n_for_month)
# 
# # create a subset of the time series that only includes the last 365 days
# LT2_daily_testing2 <- subset(ts_LT2_clean, start = length(ts_LT2_clean) - n_for_month)
# 
# autoplot(LT2_daily_training2)
# autoplot(LT2_daily_testing2)
```

```{r STL + ETS Model}

# # fit and forecast STL + ETS model to data
# ETS_fit2 <-  stlf(LT2_daily_training2, h = 30)
...
# 
# # check the MAPE
# STL_ETS_scores2 <- accuracy(ETS_fit2$mean, LT2_daily_testing2)
# print(STL_ETS_scores2)
# 
# STL_ETS_forecast2 <- ETS_fit2$mean
# print(STL_ETS_forecast2)
# 
...
```{r}
# #fitting to all of the data
# SSES_seas_year <- es(ts_LT2_clean,model="ZZZ",h=n_for,holdout=TRUE)
# plot(SSES_seas_year)
# checkresiduals(SSES_seas_year)
# SSES_scores_year <- accuracy(SSES_seas_year$forecast,LT2_daily_testing) 
# seas_scores <- as.data.frame(rbind(SSES_scores_year))
# row.names(seas_scores) <- c("SSES")
# 
# #plotting year forescast for stl + ets and sses
# autoplot(ts_LT2_clean) +
#   autolayer(SSES_seas_year$forecast, series="SSES") +
...
# n_for_month <- 30 #play with this, change to 12, 23, 2 and check of the best model is still the same
# nobs_LT2 <- nrow(LT2_depth)
# first_m_LT2 <- month(start_LT2)
# first_y_LT2 <- year(start_LT2)
# 
# ts_LT2_month <- subset(ts_LT2_clean, end = length(ts_LT2_clean) - n_for_month)
# last_obs <- subset(ts_LT2_clean, start = length(ts_LT2_clean) - n_for_month)
# 
# #fitting to all of the data
# SSES_seas <- es(ts_LT2_month,model="ZZZ",h=n_for_month,holdout=TRUE)
# plot(SSES_seas)
# checkresiduals(SSES_seas)
# 
# #check model accuracy
# SSES_scores <- accuracy(SSES_seas$forecast,last_obs) #why is this forecasting for a year? 
# 
# # create a dataframe 
# seas_scores <- as.data.frame(rbind(SSES_scores))
# row.names(seas_scores) <- c("SSES")
# 
# #could add more models to this, and more results in the above dataframe to compare MAPE values
# autoplot(ts_LT2_clean,xlim = c("2020-01-01", "2020-06-30")) +
...
#   theme_light()
#   #
#   #xlim(as.Date("2020-01-01"), as.Date("2020-06-30"))
# 
# #plotting fit to og
# autoplot(ts_LT2_clean, series="original", color='black')+
#   autolayer(SSES_seas$fitted, series='SSES') +
#   theme_light()+
#   labs(y='Depth to GW (m)', 
#        title='Model fit to original data',
#        x='Year')
#   
```
